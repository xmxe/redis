 ...
 * 比如抽奖系统扣奖品库存的时候，初始库存=总的库存数-已经发放的奖励数，但是如果是异步发奖，需要等到MQ消息消费完了才能重启redis初始化库存，
 * 否则也存在库存不一致的问题。
 */
...
public class RedisStockService {
	...
	public long stock(String key, long expire, int num, IStockCallback stockCallback) {
		...
		if (stock == UNINITIALIZED_STOCK) {
			...
			try {
				// 获取锁
				lockid = redisLock.lock(redisTemplate,key,1000L);
				if (lockid != null) {
					// 双重验证，避免并发时重复回源到数据库
					stock = stock(key, num);
					if (stock == UNINITIALIZED_STOCK) {
				...
				redisLock.unlock(redisTemplate,key,lockid);
			}
